...
export default function AIChatInterface() {
  ...
  return (
    <div className="flex flex-col justify-center items-center space-y-4 mx-auto lg:p-4 w-full max-w-4xl h-full">
      <ChatContainer
        className={cn("relative flex-1 space-y-4 pe-2 pt-10 md:pt-0 w-full", {
        ...
        {messages.map((message, index) => {
          ...
          return (
            <Message
              ...
              <div
                className={cn("flex-1 max-w-[85%] sm:max-w-[75%]", {
                  "justify-end text-end": !isAssistant
                })}>
                {isAssistant ? (
                  <div className="space-y-2">
                    <div className="bg-muted p-4 border rounded-lg text-foreground prose">
                      <Markdown className={"space-y-4"}>{message.content}</Markdown>
                    </div>
                    <MessageActions
                      className={cn(
                        "flex gap-0 opacity-0 group-hover:opacity-100 transition-opacity duration-150",
                  ...
                  <div className="flex flex-col items-end space-y-2">
                    ...
                    {message.content ? (
                      <>
                        <MessageContent className="inline-flex bg-primary text-primary-foreground">
                          {message.content}
                        </MessageContent>
                      </>
                    ) : null}
                  ...
                  <MessageContent className="inline-flex bg-primary text-primary-foreground text-start">
                    {message.content}
                  </MessageContent>
                )}
              </div>
      ...
      {!isFirstResponse && (
        <div className="mb-10">
          <div className="hidden md:block mask-b-from-100% mask-radial-from-0% mx-auto -mt-36 w-72 mask-radial-[50%_50%]">
            <Lottie className="w-full" animationData={aiSphereAnimation} loop autoplay />
          </div>

          <h1 className="font-medium text-2xl lg:text-4xl text-center leading-normal">
            Good Morning, Toby <br /> How Can I{" "}
            <span className="bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-300 text-transparent">
              Assist You Today?
            </span>
          </h1>
        </div>
      ...
      <div className="bg-primary/10 p-1 pt-0 rounded-2xl w-full">
        ...
        <Input
          ...
          className="shadow-none p-0 border-0 w-full overflow-hidden">
          {files.length > 0 && (
          ...
          <PromptInputTextarea placeholder="Ask me anything..." className="p-4 min-h-auto" />

          <PromptInputActions className="flex justify-between items-center gap-2 p-3">
            <div className="flex items-center gap-2">
              <PromptInputAction tooltip="Attach files">
                <label
                  htmlFor="file-upload"
                  className="flex justify-center items-center hover:bg-secondary-foreground/10 rounded-2xl size-8 cursor-pointer">
                  <input
                    type="file"
                    multiple
      ...
      {!isFirstResponse && (
        <div className="relative flex flex-col justify-center items-center space-y-2 w-full">
          <div className="top-0 left-0 absolute w-full h-[70px]">
            {showCategorySuggestions ? (
              ...
              <div className="relative flex flex-wrap justify-start items-stretch gap-2 w-full">
